...
final class ExcelStore implements AutoCloseable {
    ...
    private static final Map<String, List<Object>> HEADERS = new LinkedHashMap<>();
    static {
    ...
    State load() throws IOException {
        ...
        loaded = true;
        // 先完成升级再显示菜单；备份也不能继续保留明文。
        if (saved && (state.legacyPasswords || backupNeedsUpgrade())) {
            save(state.admins, state.customers, state.goods);
    ...
    void save(List<Administrators> admins, List<Customer> customers, List<Goods> goods) throws IOException {
        ...
        state.goods = goods;
        state.lastOrderId = Order.getLastOrderId();
