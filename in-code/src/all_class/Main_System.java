...
public class Main_System {
    ...
    private static ExcelStore store;
    ...
    public static void main(String[] args) {
        ...
        try (ExcelStore excelStore = new ExcelStore(directory)) {
            store = excelStore;
            System.out.println("Excel 数据文件：" + directory.resolve("supermarket-data.xlsx"));
            loadAllData();
            runMenu();
    ...
    private static void runMenu() {
        System.out.println("=========== 欢迎使用购物管理系统 3.0 ===========");
        while (true) {
            showMainMenu();
...
// 保存完整数据；失败后停止程序，避免继续使用尚未提交的内存数据。
    ...
    public static void loadAllData() {
        try {
            ExcelStore.State state = store.load();
            admins = state.admins;
