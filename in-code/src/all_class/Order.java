...
public class Order implements java.io.Serializable {
    ...
    // 编号由所属系统分配；创建或读取订单不会改变其他系统的状态。
    public Order(int orderID, Customer customer, List<Goods> goods, Map<String, Integer> conutMap, String paymentMethod){
        if (orderID <= 0) throw new IllegalArgumentException("订单编号必须为正数");
    ...
    // 数据库恢复入口，不生成新编号，也不重复累计顾客消费。
    static Order restore(int id, Customer customer, List<Goods> goods, Map<String, Integer> counts,
                         String paymentMethod, Date buyTime, double total) {
        if (id <= 0 || customer == null || goods == null || goods.isEmpty() || counts == null
                || goods.size() != counts.size() || buyTime == null || paymentMethod == null)
            throw new IllegalArgumentException("订单数据不完整");
        Set<String> ids = new HashSet<>();
        for (Goods g : goods) {
            Integer quantity = counts.get(g.getGoodsID());
            if (quantity == null || quantity <= 0 || !ids.add(g.getGoodsID()))
                throw new IllegalArgumentException("订单明细无效");
        }
        Order order = new Order(id, customer, goods, counts, paymentMethod);
        if (!Double.isFinite(total) || total < 0 || !Double.isFinite(order.totalAmount)
                || Math.abs(total - order.totalAmount) > 0.000001)
            throw new IllegalArgumentException("订单金额无效");
        order.buyTime = new Date(buyTime.getTime());
    ...
    public static Order fromFileLine(String line, Map<String, Customer> customers) {
        ...
        Order order = new Order(id, customer, items, counts, p[2]);
